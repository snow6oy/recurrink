  ...
  def effect(self):  # effect because generate is a subset
    ...
    if self.svg.selection and self.is_recurrence():
      ...
      requested = {
        ...
        'shape_size': self.options.size,
        'shape_facing': self.options.facing,
        'top': self.options.top,
        'stroke_width': int(self.options.width)
      }
      shapes = draw.get_elems_by_id(self.svg, searchId)[0]
      if requested['shape'] == 'triangle' and requested['shape_size'] == 'large':
        self.msg('Large triangles are not possible, ignoring this request.')
      elif shapes is not None:
        # update the background style, but only if we're given a new value
        bg = draw.set_background(searchId, self.options.bg, self.svg) 
        draw.set_foreground(shapes, requested)
        elem.style['stroke-width'] = svg.unittouu(self.options.width)
        # add the top elems last
        if requested['top']:
          draw.sort_groups(searchId, self.svg, svg)
      else:
        self.msg(f"group id={searchId} not found")
