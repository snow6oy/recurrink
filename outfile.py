...
class LinearSvg(Svg):
  ...
  def wireframe(self, cells=None):
    ''' preview shapes generated by Flatten in order to write meander.conf
        <polygon points="0,100 50,25 50,75 100,0" />
    '''
    inner_p = list()
    if cells:
      todo = self.makeGeominks(cells=cells)
    else:
      b1 = self.blockify()[0]
      #pp.pprint(b1)
      todo = self.makeGeominks(block=b1)
    f = Flatten()
    f.run(todo)
    s = f.stats
    print(f"""
...
TOTAL {len(f.done)}""")
  ...
  def make(self, meander_conf=dict(), cell_conf=list()):
    ...
    for block in blocks[:1]:  # layout blocks across the grid
      if len(cell_conf):
        todo = self.makeGeominks(cells=cell_conf)
      else:
        todo = self.makeGeominks(block=block)
      f = Flatten()
      f.run(todo)
      '''
      print(len(f.done))
...
TOTAL {len(f.done)}""")
  ...
  def makeGeominks(self, block=list(), cells=list()):
