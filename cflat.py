...
class Test(unittest.TestCase):
  ...
  def setUp(self):
    self.models = {
      ...
      'bossa': 'a2397e60976e01cba87a1e9e5467df2d',
  ...
  def test_a(self, model=None, line=False):
    ...
    if model: 
      ...
      clone  = Path(f'/tmp/recurrink/{model}.txt')
      ver    = 1 # it doesnt matter

      if not clone.is_file():
        print(f'cloning {digest}')
        celldata  = view.read(digest=digest, output=list())
        tf.conf(model, config.friendly_name[ver]) # conf state is a symlink
